...
- [Findings](#findings)
  ...
  - [\[M-2\] Rebase , fee on transfer , and ERC777 token break protocol invariants](#m-2-rebase--fee-on-transfer--and-erc777-token-break-protocol-invariants)
  - [Low](#low)
...
**Proof of Concept:** 
1. User wants to sell 100 pool tokens
2. User calls `sellPoolTokens` with `poolTokenAmount = 100`
3. The function calls `swapExactOutput` with `outputAmount = 100`
4. The function should call `swapExactInput` with `inputAmount = 100`
5. The user receives the wrong amount of tokens
6. The protocol receives the wrong amount of tokens
7. The protocol invariant is broken
8. The protocol is drained of funds
9. The protocol is dead
...
## [M-2] Rebase , fee on transfer , and ERC777 token break protocol invariants

**Description:** The protocol is not designed to work with tokens that have a fee on transfer, or tokens that are rebased. This is because the protocol relies on the invariant of `x * y = k` to function.

**Impact:** The protocol will not function as expected with these tokens.

**Recommended Mitigation:** The protocol should not be used with these tokens.
